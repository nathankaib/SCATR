...
c                                      (2d real array)
c                 dt            ==>  time step
c                 iclose        ==>  array telling which tp's enter hel. zone
c                 nclose        ==>  total number of tp's in hel. zone
c     
c
c
c             Output:
...
c
c
c Remarks: Based on rmvs2_step.f...changed to include variable steps
c Authors:  Hal Levison 
c Date:    7/10/96
c Last revision: 8/18/08 NAK


      subroutine rmvs3_step(i1st,time,nbod,npl,ntp,mass,j2rp2,j4rp4,xh,
     &     yh,zh,vxh,vyh,vzh,idt,xht,yht,zht,vxht,vyht,vzht,istat,rstat,
     &     dt,tinc,fopenstat,nclose,iclose,lclose,rmin,rmax,rmaxu,qmin,
     &     rplsq,rcrit,fluxdist)

      include '../swift.inc'
      include '../rmvs/rmvs.inc'

c...  Inputs Only: 
      integer nbod,ntp,i1st,istart,npl
...
c...  Internals
      ...
      real*8 vxend(NPLMAX),vyend(NPLMAX),vzend(NPLMAX),masspl(NPLMAX)
      Integer nenc(NPLMAX),itpenc(NTPMAX,NPLMAX),ienc(NTPMAX)
      integer iclose(NTPMAX)
      integer istattmp(NTPMAX,NSTAT),isperi(NTPMAX)

c...  extra variables for my changes
      character*80 fopenstat
 
c...  new crap added in to do tsteps efficiently
      integer nclose,k,g
      integer idc(NTPMAX),cind(NTPMAX)
      real*8 xc(NTPMAX),yc(NTPMAX),zc(NTPMAX)
      real*8 vxc(NTPMAX),vyc(NTPMAX),vzc(NTPMAX),msys
      real*8 dtnew
      integer istatc(NTPMAX,NSTAT),nloop
      real*8 rstatc(NTPMAX,NSTATR),qmin,rplsq(NPLMAX),rmin,rmax
      real*8 rmaxu,rcrit,mtot
      real*8 fluxdist
      logical*2 lclose
...
c     recording which particles get close to the planetary region and original
c     positions and velocities

      j = 1
      do i=1,ntp
         if (iclose(i).eq.1) then
            cind(j) = i
            idc(j) = idt(i)
            xc(j) = xht(i)
            yc(j) = yht(i)
            ...
            vzc(j) = vzht(i)
            do k=1,NSTAT
               istatc(j,k)=istat(i,k)
            enddo
            do k=1,NSTATR
               rstatc(j,k)=rstat(i,k)
            enddo
            j = j+1
         endif
      enddo
      
c...  moving at much smaller timesteps for stuff near the planetary
c...  region
      dtnew = dt/tinc
      ...
      call rmvs3_inner(i1st,time,nbod,npl,nclose,mass,
     &     j2rp2,j4rp4,xh,yh,zh,vxh,vyh,vzh,idc,xc,yc,zc,vxc,vyc,vzc,
     &     istatc,rstatc,dtnew,tinc,fopenstat,lclose,rmin,rmax,rmaxu,
     &     qmin,rplsq,fluxdist)

c     saving final positions of planets
      do i=1,npl
         masspl(i)=mass(i)
      enddo
      do i=npl+1,nbod
...
c     putting planetary positions in barycentric coords
      call coord_h2b(nbod,masspl,xbeg,ybeg,zbeg,vxbeg,vybeg,
     &     vzbeg,xbeg,ybeg,zbeg,vxbeg,vybeg,vzbeg,msys)

c     saving final positions of planets
      call coord_h2b(nbod,masspl,xend,yend,zend,vxend,vyend,
     &     vzend,xend,yend,zend,vxend,vyend,vzend,msys)

c...  putting data back into main arrays
      do i=1,nclose
         j = cind(i)
         idt(j) = idc(i)
         xht(j) = xc(i)
         ...
         vzht(j) = vzc(i)
         do k=1,NSTAT
            istat(j,k)=istatc(i,k)
         enddo
         do k=1,NSTATR
            rstat(j,k)=rstatc(i,k)
         enddo
      enddo
      
c     setting it up so particles in planetary region are temporarily inactive
c     and remembering which ones we change (by using iclose)
      do i=1,ntp
         if (iclose(i).eq.1.and.istat(i,1).eq.0) then
            istat(i,1)=1
            iclose(i)=-1
         endif
      enddo
     ...
     &     xht,yht,zht,vxht,vyht,vzht,istat,dt,rts,icflg,nenc,itpenc,
     &     ienc,npl+1)

c...     nenc and itpenc not used here!


c.... if no encounters just do a normal step and leave
      i1st = 0

c     putting all solar system mass at barycenter for big steps
      if(icflg.eq.0) then
         call step_kdk_tp(i1st,time,nbod,npl,ntp,mass,j2rp2,j4rp4,xbeg,
     &        ybeg,zbeg,xend,yend,zend,xht,yht,zht,vxht,vyht,vzht,istat,
     &        dt,0)
         do i=1,ntp
...
c...  do a full step
         ...
         call step_kdk_tp(i1sto,time,nbod,npl,ntp,mass,j2rp2,j4rp4,xbeg,
     &        ybeg,zbeg,xend,yend,zend,xht,yht,zht,vxht,vyht,vzht,
     &        istattmp,dt,0)
...
c     reactivating particles in the planetary region that we deactivated for 
c     big time steps
      do i=1,ntp
         if (iclose(i).eq.-1) then
            istat(i,1)=0
