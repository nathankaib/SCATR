...
c Last revision: 2/22/94
     ...
     &       vyh,vzh,idt,xht,yht,zht,vxht,vyht,vzht,rplsq,istat,rstat)
...
c...  Inputs: 
      ...
      real*8 xht(NTPMAX),yht(NTPMAX),zht(NTPMAX),rplsq(NPLMAX)
...
c...  Executable code 
      ...
      do j=1,ntp
         if(istat(j,1).eq.0) then
            ...
            if(istat(j,3).ne.0 .and. istat(j,3).lt.6) then

                 if( (rstat(j,3).gt.0.0) .and.
     &           (peri2.lt.rplsq(istat(j,3))) ) then
                   istat(j,1) = 1
                   istat(j,2) = istat(j,3)
                   write(*,*) 'Particle',idt(j),' q with respect to 
     &              Planet ',istat(j,3),' is too small at t=',time
                    rstat(j,1) = time
     ...
     &                 dt,rplsq(i),iflg,r2min)
                  if(iflg.eq.1) then
                     istat(j,1) = 1
                     istat(j,2) = i
                     write(*,*) 'Particle',idt(j),' too close to 
     &                    Planet ',i,' at t=',time
                     rstat(j,1) = time
