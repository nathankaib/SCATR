...
c Last revision: 1/20/97
     ...
     &     vyh,vzh,idt,xht,yht,zht,vxht,vyht,vzht,rmin,rmax,rmaxu,
...
c...  Inputs: 
      ...
      real*8 mass(NPLMAX),xh(NPLMAX),yh(NPLMAX),zh(NPLMAX),time
      real*8 rplsq(NPLMAX),vxh(NPLMAX),vyh(NPLMAX),vzh(NPLMAX)
...
c...  Executable code 
     ...
     &        xbt,ybt,zbt,vxbt,vybt,vzbt)
         call discard_sun(time,ntp,msys,idt,xht,yht,zht,xbt,ybt,zbt,
     &       vxbt,vybt,vzbt,rmin,rmax,rmaxu,istat,rstat)
      endif

      if(qmin.ge.0.0) then
         call discard_peri(time,ntp,idt,xht,yht,zht,vxht,vyht,vzht,
     &        qmin,istat,rstat,nbod,mass,xh,yh,zh,vxh,vyh,vzh)
      endif

      if(lclose) then
         call discard_pl(time,dt,nbod,ntp,mass,xh,yh,zh,vxh,vyh,vzh,
     &       idt,xht,yht,zht,vxht,vyht,vzht,rplsq,istat,rstat)
      endif
