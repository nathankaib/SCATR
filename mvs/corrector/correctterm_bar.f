...
c Last revision: 6/2/09
      ...
      subroutine correctterm_bar(nbod,npl,ntp,time,mass,j2rp2,j4rp4,xh,
...
c     Do planetary drift ------------------- (assumes pure kepler for pls)
...
c     Done with pl drift


c     Convert pl's to barycentric coords
      mtot = mass(1)
      xbar = 0.0
      ybar = 0.0
      zbar = 0.0
...
c     Do dk-d for tp's--------
c     drifting barycentric tp's forward
      call drift_tp(ntp,mtot,xht,yht,zht,vxht,vyht,vzht,adt,istat)

c     Get the accelerations in bary frame.
      call getacch_tp(nbod,npl,ntp,time,mass,j2rp2,j4rp4,xb,yb,zb,
     &     xht,yht,zht,istat,axht,ayht,azht,bdt,mtot,0)

c     Apply a kick for bdt 
      call kickvh_tp(ntp,vxht,vyht,vzht,axht,ayht,azht,istat,bdt)

c     drifting barycentric tp's backward
      adt=0.0-adt
      call drift_tp(ntp,mtot,xht,yht,zht,vxht,vyht,vzht,adt,istat)
c      Done with tp dk-d------------
...
c     Do planetary drift------------------- (assuming pure Kepler for pls)
      call coord_h2j(nbod,mass,xh,yh,zh,vxh,vyh,vzh,
     &          xj,yj,zj,vxj,vyj,vzj)
