...
c Last revision: 
      ...
      subroutine discard_sun(time,ntp,msys,idt,xht,yht,zht,xbt,ybt,zbt,
      ...
      do i=1,ntp
         if(istat(i,1).eq.0) then
            ...
            if( (rmax.ge.0.0) .and. (rh2.gt.rmax2) ) then
               write(*,*) 'Particle',idt(i),' too far from Sun at t=',
     &              time
               istat(i,1) = 1
               istat(i,2) = -3
               rstat(i,1) = time
            ...
            if( (rmin.ge.0.0) .and. (rh2.lt.rmin2) ) then
               write(*,*) 'Particle',idt(i),' too close from Sun at t=',
     &              time
               istat(i,1) = 1
               istat(i,2) = 1
               rstat(i,1) = time
         ...
         if( (istat(i,1).eq.0) .and. (rmaxu.ge.0.0) ) then
            ...
            if( (energy.gt.0.0) .and. (rb2.gt.rmaxu2) ) then
               write(*,*) 'Particle',idt(i),' is unbound and too far ',
