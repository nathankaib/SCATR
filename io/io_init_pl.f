...
c Last revision: 3/3/94  HFL
	...
	subroutine io_init_pl(infile,lclose,iflgchk,nbod,npl,mass,xh,
...
c Read number of planets
	read(7,*, end=10, iostat=ftest) nbod,npl
...
c and helioc. position and vel .
        if(btest(iflgchk,5))  then ! bit 5 is set
           read(7,*, end=10, iostat=ftest) mass(1),j2rp2,j4rp4
        else
           read(7,*, end=10, iostat=ftest) mass(1)
           j2rp2 = 0.0d0
           j4rp4 = 0.0d0
        endif
        read(7,*, end=10, iostat=ftest) xh(1),yh(1),zh(1)
        read(7,*, end=10, iostat=ftest) vxh(1),vyh(1),vzh(1)
        write(*,*) mass(1)
	...
	do j=2,nbod
           if(lclose) then
              read(7,*, end=10, iostat=ftest) mass(j),rpl
              write(*,*) mass(j),rpl
              rplsq(j) = rpl*rpl
           else
              read(7,*, end=10, iostat=ftest) mass(j)
              write(*,*) mass(j)
           endif
	  read(7,*, end=10, iostat=ftest) xh(j),yh(j),zh(j)
	  read(7,*, end=10, iostat=ftest) vxh(j),vyh(j),vzh(j)
