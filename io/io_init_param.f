...
c Last revision:  5/10/94  HFL
        ...
        subroutine io_init_param(infile,t0,tstop,dt,tinc,
     &     dtout,dtdump,iflgchk,rmin,rmax,rmaxu,qmin,lclose,
...
c...  Outputs: 
	integer iflgchk
	real*8 t0,tstop,dt,tinc
	real*8 dtout,dtdump
	real*8 rmin,rmax,rmaxu,qmin,rcrit
        real*8 qrec,fluxdist
        logical*2 lclose
	character*80 outfile,fopenstat,frame

c...  Internals
        logical*1 lflg(0:IO_NBITS-1)
        integer i,ierr,ftest
...
c...  Executable code 
	...
	read(7,*, end=10, iostat=ftest) t0,tstop,dt,tinc
	write(*,*) 't0,tstop,dt,tinc : ',t0,tstop,dt,tinc
	read(7,*, end=10, iostat=ftest) dtout,dtdump
	write(*,*) 'dtout,dtdump : ',dtout,dtdump
        read(7,*, end=10, iostat=ftest) (lflg(i),i=IO_NBITS-1,0,-1)
     ...
     &                '       an integer binary file be written '
        ...
        if(btest(iflgchk,4))  then ! bit 4 is set
           read(7,*, end=10, iostat=ftest) rmin,rmax,rmaxu,qmin,lclose
     ...
     &          rmin,rmax,rmaxu,qmin,lclose
        ...
        read(7,*, end=10, iostat=ftest) qrec,fluxdist
        write(*,*) 'qrec: ',qrec
        write(*,*) 'fluxdist: ',fluxdist

        read(7,*, end=10, iostat=ftest) rcrit
        write(*,*) 'rcritical: ',rcrit

        if(btest(iflgchk,0) .or. btest(iflgchk,1))  then 
           read(7,999, end=10, iostat=ftest) outfile
 999       format(a)
           write(*,*) 'outfile : ', outfile
           write(*,*) ' '
           read(7,999, end=10, iostat=ftest) frame
           write(*,*) 'output/input : ',frame
           if(  (frame(1:3).ne.'bar') .and.
     &          (frame(1:3).ne.'BAR') .and.
     &          (frame(1:3).ne.'hel') .and.
     &          (frame(1:3).ne.'HEL') ) then
              write(*,*) ' SWIFT ERROR: in io_init_param:'
              write(*,*) '    Invalid frame flag:',fopenstat(1:3),':'
           endif
        endif

        read(7,999, end=10, iostat=ftest) fopenstat
