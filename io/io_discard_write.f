...
c Last revision: 7/11/94
     ...
     &     vxh,vyh,vzh,idt,tpfrac,xht,yht,zht,vxht,vyht,vzht,istat,
...
c...              try to open as new
 ...
 1000          format(1x,1p1e23.16)
            ...
            iwrite = 1
            irem = irem + 1
            write(iu,2000) idt(i),(istat(i,j),j=1,NSTAT)
...
c        adjusting arrays to remove inactive tps
         act = 0
         do i=1,ntp
           if (istat(i,1).eq.0) then
               act=act+1
               do j=1,NSTAT
                  istat(act,j)=istat(i,j)
               enddo
               do j=1,NSTATR
                  rstat(act,j)=rstat(i,j)
               enddo
               istold(act)=istold(i)
               idt(act)=idt(i)
               tpfrac(act)=tpfrac(i)
